...
/**
 * Created by chenxiaoxuan1 on 17/3/6.
 */
...
class Solution {
    public double findMedianSortedArrays(int[] nums1, int[] nums2) {
        ...
        int median1 = (nums1.length - 1) / 2;
        int median2 = (nums2.length - 1) / 2;
        final int medianIndex = (nums1.length + nums2.length) / 2;
        final boolean isTwoNum = ((nums1.length + nums2.length) & 1) == 0 ? true : false;
        int end = isTwoNum ? medianIndex : medianIndex + 1;
        int deleteNum = 0;
        int start1 = 0;
        int start2 = 0;
        while (true) {
            if (getValue(nums1, median1) > getValue(nums2, median2)) {
                int safe = end - 1 - median1;
                safe = safe >= 0 ? safe : 0;
                int start = start2;
                if (start2 < nums2.length) {
                    if (start2 < safe) {
                        start2 = median2 < safe ? median2 : safe;
                    }
                }
                int deleteNumTemp = deleteNum + start2 - start;
                if ((deleteNumTemp < end - 1) && deleteNumTemp == deleteNum) {//至少删除一个,排除最小
                    if (getValue(nums1, start1) > getValue(nums2, start2)) {
                        median2 = median2 == start2 ? median2 + 1 : median2;
                        start2++;
                    } else {
                        median1 = median1 == start1 ? median1 + 1 : median1;
                        start1++;
                    }
                    deleteNumTemp++;
                    deleteNum++;
                ...
                deleteNum = deleteNumTemp;
                if (deleteNumTemp >= end - 1) {//进行最小筛选
                    //手动删除最小的直到删够为止
                    while (deleteNum < (end - 1)) {
                        if (getValue(nums1, start1) > getValue(nums2, start2)) {
                            start2++;
                        } else {
                            start1++;
                    ...
                    return getMedian(nums1, nums2, start1, start2, isTwoNum);
                } else {
                    deleteNum = deleteNumTemp;
                    median1 = (median1 + start1) / 2;
                    median2 = (start2 + nums2.length - 1) / 2;
                    median2 = start2 >= nums2.length ? start2 : median2;
                }
            } else {
                //安全删除
                int safe = end - 1 - median2;
                int start = start1;
                safe = safe >= 0 ? safe : 0;
                if (start1 < nums1.length) {
                    if (start1 < safe) {
                        start1 = median1 < safe ? median1 : safe;
                    }
                }
                int deleteNumTemp = deleteNum + (start1 - start);
                if ((deleteNumTemp < end - 1) && deleteNumTemp == deleteNum) {
                    if (getValue(nums1, start1) > getValue(nums2, start2)) {
                        median2 = median2 == start2 ? median2 + 1 : median2;
                        start2++;
                    } else {
                ...
                if (deleteNumTemp >= end - 1) {//进行最小筛选
                    ...
                    median2 = (median2 + start2) / 2;
                    median1 = (start1 + nums1.length - 1) / 2;
                    median1 = start1 >= nums1.length ? start1 : median1;
                }
            }
        }
    ...
    private double getMedian(int[] nums1, int[] nums2, int start1, int start2, boolean isTwoNum) {
        int[] arr = new int[4];
        int i = 0;
        if (start1 < nums1.length) {
            arr[i++] = nums1[start1++];
    ...
    private int getValue(int[] arr, int index) {
        int num = index >= arr.length ? Integer.MAX_VALUE : arr[index];
        return num;
    }
